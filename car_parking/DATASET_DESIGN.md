...
기본 benchmark는 고정 task 1–5를 episode 단위로 균등하게 수집하고,
expert/noisy/recovery를 20-episode block마다 70/25/5로 섞은 물리-state 데이터셋으로 만든다.
Expert는 Hybrid A*로 충돌 없는 전진/후진 경로를 찾고,
...
따라서 NPZ에는 아래 8차원 Markov 물리 상태만 기본 observation으로 저장한다.

```text
[x, y, cos(yaw), sin(yaw), speed, normalized_steering, health, dwell_progress]
```

Loader는 batch에서 선택된 goal마다 다음을 다시 계산해 11차원 state를 만든다.
...
- `health`: raw state의 6번 값을 그대로 복사한다. goal relabeling과 무관하다.
- `dwell_progress`: raw state에서 보존한다. 과거 이력을 숨기지 않는 Markov 상태다.
...
|---|---|---|
| `observations` | `(N, 8) float32` | health와 dwell progress를 포함한 Markov 상태 |
| `actions` | `(N, 2) float32` | normalized steering/throttle |
| `next_observations` | `(N, 8) float32` | 다음 health/dwell을 포함한 물리 상태 |
| `commanded_goals` | `(N, 5) float32` | `[x, y, cos(yaw), sin(yaw), reached]` |
...
| `task_ids` | `(N,) int8` | 고정 task 1–5 |
| `behavior_ids` | `(N,) int8` | expert/noisy/recovery/random |
| `slot_shifts` | `(N,) float32` | canonical slot에서의 연속 x shift |
...
통계로 남긴다. Planner path가 존재해도 controller rollout이 충돌하면 실패다. Dataset 생성은
pathological layout을 빠르게 reject하도록 attempt당 탐색을 5,000 expansion으로
제한한다; canonical task 1–5는 이 상한에서 모두 계획된다. Train start jitter는 nominal path를 다시 탐색하지 않고 tracker의 복원 성능을
검증한다. Validation은 held-out jitter pose에서 새 경로를 계획해 scenario
feasibility까지 검사한다.

### 3. Behavior mixture

기본 benchmark는 `mixture` 파일이다.

- 70%: 성공한 deterministic expert trajectory
- 25%: correlated action noise가 있는 성공 trajectory
- 5%: 초반 강한 perturbation 뒤 현재 pose에서 재계획하는 recovery trajectory

Recovery trajectory는 성공 여부와 관계없이 완전한 episode를 보존한다. Perturb 후
재계획이 실패해도 episode를 reject하지 않고 기존 tracker로 끝까지 진행한다. 따라서
성공 expert와 즉시 사망 random 사이에 timeout, near-miss, recovery가 포함된다.
`expert`와 `noisy` 성공 전용 파일, 모든 실패를 보존하는 `random` 파일은
각각 진단/ablation 용도로 별도 생성한다. 모든 transition에는 `behavior_ids`를
저장한다.

## Task와 split 균형

Transition 수가 아니라 고정 task 1–5의 episode 수를 먼저 20%씩 균등
배분한다. 두 parallel variant도 별도 task이므로 각각 독립적인 20%를 받는다.
긴 T-forward가 transition 수를 많이 차지해도 loader가 task를 먼저 균등
선택한 뒤 해당 task의 valid transition을 뽑아 batch 편향을 막는다.

Train은 canonical goal 20%와 `slot_shift_x ∈ [-0.005, 0.005]`를 섞는다.
Validation은 별도 seed/start jitter와 함께 shift 절댓값 `[0.0075, 0.01]`을
사용하므로 goal 위치가 train과 겹치지 않는다. 고정 평가는 shift 0이다.
...
않으므로 v2에서는 사용하지 않는다.
`reached=1`인 commanded goal은 실제 dwell 완료를 요구하고, future goal의
reached 좌표는 해당 future transition의 Markov dwell progress를 사용한다.
...
- canonical task 1–5 expert가 모두 성공
- 기본 dataset jitter(`±0.005`, `±1°`) 25회에서 전체 성공률 `>= 70%`,
  task별 성공률 `>= 60%`
- 성공 전용 expert 파일의 contact/death rate `= 0%`
- mixture가 recovery 성공·timeout·death episode를 삭제하지 않음
- task 1–5 episode 수 차이 `<= 1`
- sampled batch의 task 분포가 균등
- 모든 episode 마지막에만 `terminals=True`
- death와 timeout label이 동시에 켜지지 않음
- schema가 `parking_v2`, raw state 8-D, goal 5-D, reconstructed state 11-D
- 같은 raw state에 두 goal을 적용했을 때 physical feature, health,
  dwell progress는 동일하고 6–8번 goal-relative feature만 달라지는 unit test

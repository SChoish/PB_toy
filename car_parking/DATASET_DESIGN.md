...
따라서 NPZ에는 아래 6차원 물리 상태만 기본 observation으로 저장한다.
...
- `collision`: raw transition의 collision flag
...
|---|---|---|
| `observations` | `(N, 6) float32` | goal-independent 물리 상태 |
| `actions` | `(N, 2) float32` | normalized steering/throttle |
| `next_observations` | `(N, 6) float32` | 다음 물리 상태 |
...
| `collisions` | `(N,) bool` | absorbing collision failure |
...
Whole episode만 저장한다. `terminals`는 collection boundary이고 `collisions`만
absorbing failure다. Timeout은 Bellman absorbing으로 취급하지 않는다.
...
- collision: mask 0인 absorbing failure
...
- expert collision rate `<= 1%`
- noisy-recovery 성공률 `>= 70%`
- maneuver별 episode 수 편차 `<= 2%`
- 모든 episode 마지막에만 `terminals=True`
- collision과 timeout label이 동시에 켜지지 않음
- 같은 raw state에 두 goal을 적용했을 때 0–5번 physical feature는 동일하고
  6–8번 goal-relative feature만 달라지는 unit test

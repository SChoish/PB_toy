#!/usr/bin/env bash
# Confirmed next queue after PathBridger common4→1M:
#   5 agents × (CarRace ice/grav/anti_grav × lap_1p/2p/4p × noisy/random
#              + SwingBy planet/blackhole × noisy/random
#              + CarParking noisy/random)
#   SIZE=100k STEPS=50000 K=10 h_a=2 seed=0  PACK=32 / GPU×2
#
# CarParking train may still be WIP — those rows stay in the queue and are
# skipped until `python -m car_parking.train` + 100k NPZ exist.
#
# Usage:
#   DRY_RUN=1 bash scripts/run_queue_100k_k10.sh
#   WAIT_FOR_PB=1 bash scripts/run_queue_100k_k10.sh   # poll until no common4 main.py
#   bash scripts/run_queue_100k_k10.sh
set -euo pipefail
...
LOGDIR="$ROOT/nohup_logs/queue_100k_k10"
...
SWING_POLICIES=(noisy random)
PARK_POLICIES=(noisy random)
...
# Cache once — importing train modules per job is too slow.
HAS_CAR_TRAIN=0
HAS_SWING_TRAIN=0
HAS_PARK_TRAIN=0
...
wait_for_pathbridger() {
  log "WAIT_FOR_PB: blocking until no common4_data500k main.py"
  while pgrep -af 'main.py.*common4_data500k' | grep -v pgrep >/dev/null 2>&1; do
    sleep 60
  done
  log "WAIT_FOR_PB: PathBridger train gone; starting queue"
}
...
# Avoid matching the `sg nvidia -c "...python -u -m ..."` wrapper (root),
# which made /proc/*/environ unreadable and left every GPU looking empty.
_train_pids() {
  pgrep -f '/python -u -m (car_race|swingby|car_parking)\.train' 2>/dev/null || true
}

# gpu_index -> uuid (filled once in main)
declare -A GPU_UUID=()
init_gpu_uuids() {
  local idx uuid
  GPU_UUID=()
...
count_on_gpu() {
  ...
  # Count our train processes that nvidia-smi reports on this GPU.
  local apps
...
pick_gpu() {
  ...
  # A JAX process can create hundreds of threads.  PACK is only a GPU cap;
  # the cgroup PID budget is the stricter limit on this host.
...
log "QUEUE_100k_k10 jobs=${#JOBS[@]} pack=$PACK gpus=${GPUS} steps=$STEPS K=$K h_a=$HA size=$SIZE seed=$SEED dry=$DRY_RUN"
log "breakdown: car=$((3*3*2*5)) swing=$((2*2*5)) park=$((2*5)) total=${#JOBS[@]}"
log "gpu_uuids: $(for g in "${GPU_ARR[@]}"; do echo -n "$g=${GPU_UUID[$g]:-?} "; done)"

# Background: periodically export running/finished evals + stdout into PB_logs.
...
if [[ "$WAIT_FOR_PB" == "1" && "$DRY_RUN" != "1" ]]; then
  wait_for_pathbridger
fi
if [[ "$DRY_RUN" != "1" ]]; then
  log "GPU preflight"
...
log "=== QUEUE_100k_k10_DONE === deferred=$(wc -l <"$LOGDIR/deferred.txt") (see deferred.txt)"

# Final sync of eval/stdout into PB_logs (watcher also runs periodically).
...
# Chain: 10k lap248 matrix (anti_grav remain + full ice/grav/anti_grav) + handoff + 1k NT sweep
# Override with NEXT_QUEUE='...' or disable with NEXT_QUEUE=skip
if [[ "${NEXT_QUEUE:-}" == "0" || "${NEXT_QUEUE:-}" == "skip" ]]; then
  ...
  log "NEXT_QUEUE: scripts/run_queue_post_100k.sh"
  bash "$ROOT/scripts/run_queue_post_100k.sh"
  log "NEXT_QUEUE finished"
fi

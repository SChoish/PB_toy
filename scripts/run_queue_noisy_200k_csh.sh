...
init_gpu_uuids() {
  ...
  while IFS=',' read -r idx uuid; do
    ...
    GPU_UUID["$idx"]="$uuid"
  done < <(bash -c 'nvidia-smi --query-gpu=index,uuid --format=csv,noheader' 2>/dev/null || true)
}
...
count_on_gpu() {
  ...
  local apps
  apps=$(bash -c 'nvidia-smi --query-compute-apps=gpu_uuid,pid --format=csv,noheader' 2>/dev/null || true)
...
run_job() {
  ...
  (
    ...
    bash -c "export CUDA_VISIBLE_DEVICES=$gpu \

...
once() {
  ...
  if [[ "$UPDATE_RESULTS_MD" == "1" ]]; then
    ...
    "$PY" -u scripts/parse_pb_noisy100k_nt_sweep.py --agents pbf || log "WARN nt sweep parse failed rc=$?"
    log "parse_pb_noisy10k_nt_sweep"
    "$PY" -u scripts/parse_pb_noisy100k_nt_sweep.py --size-suffix 10k --agents pbf,pbg \
      || log "WARN 10k nt sweep parse failed rc=$?"
  ...
  if [[ "$PUSH" == "1" ]]; then
    ...
    if [[ -d "$LOGS_ROOT/pb_toy" ]]; then
      ...
      for f in \
        ...
        PB_toy_nt_sweep_noisy10k_200k.md \
        PB_toy_nt_sweep_noisy10k_200k.json
      do

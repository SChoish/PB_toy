...
``phi`` / ``auto`` / ``goal_phi`` are defined for these families
(distinguished by ``env_name``):
  ...
  - **car_race** / **swingby** (PB_toy): task goal prefix ``(0, 1, 2, 3)`` —
    car_race ``[x,y,progress,direction]``, swingby ``[gx,gy,gvx,gvy]``.

If ``phi_goal_obs_indices`` is non-empty it is honored at runtime (explicit
override). When omitted/empty, channels come from ``env_name`` as above.
"""
...
# PB_toy: actor φ is the 4-D task-goal prefix of the observation / goal vector.
_TOY_GOAL4_INDICES = (0, 1, 2, 3)
...
def _env_goal_phi_kind(env_name: str | None) -> str:
    ...
    raise ValueError(
        ...
        "'scene', 'cube', 'car_race', or 'swingby'."
    )
...
def _parse_scene_compact_layout(obs_dim: int) -> tuple[int, int, int]:
    ...
    if len(solutions) > 1:
        # OGBench scene-play-v0 default: 1 cube, 2 buttons, 2 states per button.
        preferred = (1, 2, 2)
        if preferred in solutions:
            return preferred
        raise ValueError(
...
def infer_phi_goal_obs_indices(env_name: str | None, obs_dim: int | None = None) -> tuple[int, ...]:
    ...
    ``goal_representation(..., 'phi', ...)`` picks channels from explicit indices
    when provided, otherwise from ``env_name`` / goal tensor.

    Maze envs always return ``(0, 1)`` (planar goal). Toy car_race/swingby
    return ``(0, 1, 2, 3)``. Puzzle/cube need ``obs_dim`` to validate the compact
    layout. Scene returns ``()`` (φ is assembled in ``scene_oracle_phi_from_goals``).
    """
    ...
    if kind in ('car_race', 'swingby'):
        return _TOY_GOAL4_INDICES
    if obs_dim is None:
        return ()
    dim = int(obs_dim)
    if kind == 'puzzle':
    ...
    *,
    ...
    if idxs:
        if min(idxs) < 0 or max(idxs) >= dim:
            raise ValueError(
                f'{where}: phi_goal_obs_indices={idxs} out of range for obs_dim={dim}.'
            )
        return
    try:
        kind = _env_goal_phi_kind(env_name)
    ...
    if kind in ('car_race', 'swingby'):
        if dim < 4:
            raise ValueError(
                f'{where}: goal_representation={mode_l!r} for env={env_name!r}: '
                f'obs_dim={dim} must be >= 4 for toy goal prefix (0,1,2,3).'
            )
        return
    raise AssertionError(f'unreachable phi kind={kind!r}')
...
def goal_representation(
    ...
    *,
    ...
    explicit = normalize_phi_goal_obs_indices(phi_goal_obs_indices)
    if explicit:
        if max(explicit) >= obs_dim:
            raise ValueError(
                f"goal_representation='phi': phi_goal_obs_indices={explicit} "
                f'out of range for goal_dim={obs_dim}.'
            )
        return jnp.take(goals, jnp.asarray(explicit, dtype=jnp.int32), axis=-1)
    ...
    if kind in ('car_race', 'swingby'):
        if obs_dim < 4:
            raise ValueError(
                f"goal_representation='phi' for env={env_name!r}: obs_dim={obs_dim} must be >= 4 "
                'for toy goal prefix (0, 1, 2, 3).'
            )
        take = jnp.asarray(_TOY_GOAL4_INDICES, dtype=jnp.int32)
        return jnp.take(goals, take, axis=-1)
